...
| **Devices** | A `CAP_IO_DEVICE` names **one device** in a boot-time table (PCI bus-0 scan plus the non-enumerable legacy platform hardware) and confers only that device's frames, port ranges and interrupt lines. Two ring-3 drivers: `console_server` and `netd`, a virtio-net driver proved by a real ARP exchange. **No** IOMMU, so a bus-mastering device's DMA is bounded by nothing; no MSI, no bridge walk, and a PCI interrupt line cannot yet be delivered to ring 3 |
| **Network** | `netd` drives virtio-net from ring 3 holding one device capability and one untyped region — enough Ethernet to prove the device is driven. **No** ARP table, IP, TCP or socket capabilities yet |
