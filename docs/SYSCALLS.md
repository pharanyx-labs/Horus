...
**Interface**: Syscall number in `rax`; arguments in `rbx, rcx, rdx, rsi, rdi`. Arguments and the return value are 64-bit — they carry user pointers, and `SYS_BRK`/`SYS_SBRK` return an address. The kernel services syscalls through its `int 0x80` handler (`interrupt_handler64` → `syscall_handler`), which dispatches via a descriptor table: for syscalls with a single fixed authorising capability it is enforced centrally before the handler runs, and any syscall number with no table entry fails closed.
...
| Number | Name              | Description                                                  | Required Capability                          | Notes |
|--------|-------------------|--------------------------------------------------------------|----------------------------------------------|-------|
| 28     | `SYS_SPAWN`       | Spawn a named embedded binary as a new task                 | endpoint WRITE\|EXEC (slot 3)                | Runs the ELF load in the kernel address space; hands the caller a `CAP_TCB` to the child. `rdx` = one-word arg; `rsi`/`rdi` = argv/argc (marshalled onto the child's stack). `sys_spawn_named[_arg\|_argv](…)` |
...
| 64     | `SYS_EXEC_NAMED`  | Replace the caller's own image with a named embedded binary | endpoint WRITE\|EXEC (slot 3)                | Same task id and cspace (capabilities survive, POSIX-style); `rsi`/`rdi` = argv/argc marshalled onto the fresh stack. Does not return on success. `sys_exec_named[_argv](…)` |
| 70     | `SYS_SPAWN_IMAGE` | Spawn a child from a program image supplied in the caller's own memory (execve-from-fd) | endpoint WRITE\|EXEC (slot 3) | `rbx`/`rcx` = image/len; the caller reads the image from a file via the `fs_server`, the kernel validates it with the same loader a named binary uses (Horus `.bin` header or bare ELF; `arm_image_from_user` → `try_elf_load`). `rdx` = one-word arg; `rsi`/`rdi` = argv/argc. Returns the child pid. `sys_spawn_image(…)` |
...
| 66     | `SYS_SIGNAL`      | Send an async signal to another task                        | `CAP_TCB` to the target (or `CAP_USER` admin)| Same authority as `SYS_KILL`. Queues `signum` (1..31); redirected into the target's `SYS_SIGACTION` handler on its next return to ring 3 (signal # in `rbx`). Unhandled — or the uncatchable `SIG_KILL` (9) — takes the default terminate action |
...
| 73     | `SYS_IPC_SENDER`  | Kernel-attested identity of an endpoint's last sender | endpoint READ (slot 3) | Returns the sender's uid and (via `*rcx`) gid, taken from `tasks[last_sender]` — set at login, not from the message — so a server (e.g. the `fs_server`) learns who a client is without trusting the client. Returns `(uint32_t)-1` if there is no valid sender |
...
| 26     | `SYS_WAIT_NOTIFY` | Wait for a notification badge                | endpoint READ (slot 3)     | Returns a pending badge, else blocks; badge delivered in `rbx`. Proven by `make smoke-notify` |
...
| Number | Name               | Description                                  | Required Capability                 | Notes |
|--------|--------------------|----------------------------------------------|-------------------------------------|-------|
| 54     | `SYS_SIGACTION`    | Register/clear this task's own fault/signal handler | None (self only)             | Handler vaddr validated to the user code window (safe Rust); 0 clears. On a ring-3 fault the kernel enters the handler (signal # in `rbx`, fault addr in `rcx`) instead of killing the task |
