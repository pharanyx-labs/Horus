...
| `META_CRASH_DROP_ONE=1` | Loses one block's crypto metadata on its way to the disk, modelling a bounded cache evicting a dirty entry without writing it back (failure mode E1 in `docs/design/meta-cache-merkle.md`). It clears the in-RAM entry as well as skipping the write, because skipping alone does not reproduce: a complete mirror rewrites the whole meta block from RAM on the next flush and heals it. | `make smoke-meta-crash-control`, which requires `METACACHE: FAIL block` on the second boot; `make smoke-meta-crash` must go red under it. Targets the Nth flush rather than a physical block number — the self-test writes logical blocks and cannot know which physical block the allocator returns, so an address would be a guess that might fall outside the working set |
