...
| **Storage crypto** | Per-`(inode, block)` AEAD subkeys, Merkle rollback tree (partial rollback only — the root lives in the superblock it protects); key material never leaves the kernel |
