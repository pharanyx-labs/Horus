...
Capability revocation carries Kani proofs. `.github/workflows/ci.yml` runs 96 jobs, most of
...
gating, or exempted with a written reason (**[C-6]**). The intended set is 98 of its 101 contexts,
...
| **Devices** | A `CAP_IO_DEVICE` names **one device** in a boot-time table (PCI bus-0 scan plus the non-enumerable legacy platform hardware) and confers only that device's frames, port ranges and interrupt lines. **No** IOMMU, so a device's DMA is bounded by nothing; no MSI, no bridge walk, and `console_server` is still the only driver |
...
2. **QEMU integration self-tests** — the bulk of CI's 96 jobs; each boots a purpose-built
