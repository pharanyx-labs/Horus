...
`tools/mkheadered` wraps it. The **unstripped `.pie.elf` is kept** — marked `.SECONDARY` so make
...
| Target | Asserts |
|---|---|
| `smoke-captest` | Unheld capabilities, post-revoke use, and bad input are all refused (133 checks — read the count off `CAPTEST: PASS <n> checks` on the wire, not from here) |
