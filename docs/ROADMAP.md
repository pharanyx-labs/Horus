...
- **4.2 🚧 Gate the security tests — [C-6].** `strict_required_status_checks_policy` is now
  ...
  **The mechanism was closed on 2026-08-16.** `.github/ci-gating.yml` is a checked-in decision
  ...
  The intended set is **75 required, 4 exempted** (76 jobs, 79 contexts — re-derive it with
...
| ✅ | 84 `smoke-*` targets (`grep -c '^smoke-[a-z0-9-]*:' Makefile`), nearly all QEMU integration self-tests, several adversarial, and 11 of them control arms that must reproduce a defect |
