...
| S25 | Which syscalls a test actually exercises is measured, not assumed | `SYSCALL_COVERAGE=1` records first entry into each handler body; `.github/syscall-coverage.yml` classifies all 76 implemented syscalls as covered or uncovered-with-a-reason; `tools/check_syscall_coverage.py` diffs the measured union against it. Currently **51 of 76** — the property is that the number is decided and stable, not that it is 76 | `syscall-coverage`, a required check (`make smoke-syscall-coverage`). Falsified four ways: an unclassified syscall, a covered one that stopped being entered, an uncovered one that started being entered, and a serial log carrying no `SYSCOV` lines at all |
