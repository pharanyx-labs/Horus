...
- **4.2 🚧 Gate the security tests — [C-6].** `strict_required_status_checks_policy` is now
  ...
  **The mechanism was closed on 2026-08-16.** `.github/ci-gating.yml` is a checked-in decision
  ...
  The intended set is **82 required, 4 exempted** (83 jobs, 86 contexts — re-derive it with
...
| ✅ | 95 `smoke-*` targets (`grep -c '^smoke-[a-z0-9-]*:' Makefile`), nearly all QEMU integration self-tests, several adversarial, and 17 of them control arms that must reproduce a defect |
