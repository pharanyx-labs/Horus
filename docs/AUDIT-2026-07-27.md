...
**Predecessor.** `docs/AUDIT-2026-07.md` (findings A1–A4, P1–P5). This audit supersedes it
and re-verifies its remediations.
...
| Dimension | Rating | Direction |
|---|---|---|
| Kernel implementation security | **Moderate–High risk** | Improving fast |
| Capability model *as implemented* | **High risk** (one systemic bypass) | Regressed vs. intent |
| Memory-safety posture | **Low risk** | Strong |
| Boot / supply-chain integrity | **Low risk** | Strong |
| Repository & CI/CD security | **Low risk** | Strong |
| Governance / review process | **High risk** | Static |
| **Combined posture** | **Moderate–High risk** | — |
...
should be a `GS`-based per-CPU block (`swapgs` + `%gs:offset`).
...
### [C-1] IPC endpoints are ambient: any task may send, receive, and forge replies on any endpoint — *Critical*
...
**Regression test to add** (`userspace/captest.c`): assert that a task holding an endpoint
capability for object *N* is refused `send`/`recv` on every other endpoint, and that a task
with no endpoint capability is refused all IPC. This is the test whose absence let the
defect stand.

---

### [C-2] Notification objects are ambient — *High*
...
### [I-1] Ambient `uid == 0` authority parallel to the capability system — *Medium-High*
...
**Fix:** replace each `uid == 0` test with a distinct capability type
(`CAP_KERNEL_LOG`, `CAP_BOOT_MODULE`, `CAP_OBJECT_STORE`) delegated by `init`, so the
capability graph is a complete description of authority. This is prerequisite work for any
meaningful confinement or MLS story later.

### [I-2] `h_sbrk`/`h_brk` truncate 64-bit heap arithmetic to 32 bits — *Medium*
...
### [I-4] `h_task_info` leaks another task's `%rip` to any root task — *Medium*
...
observed task by directly revealing a code address. **Fix:** zero `eip` for
`tid != get_current_task()`, or gate it behind an explicit debug capability.
...
**[C-3]**'s fix. Removes an MMIO read from the hottest kernel path.
...
*Audit conducted 2026-07-27 against commit `a4c7b1a`. Findings [C-3] fixed in the
accompanying change; all other findings are open and tracked in `docs/ROADMAP.md`.*
