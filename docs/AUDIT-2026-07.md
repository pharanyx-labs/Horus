...
| **A2** | Capability engine | `SYS_CAP_GRANT` bypasses the locked cap-write discipline (no reserved-slot floor, no `caps_in_use` accounting, copies full rights incl. `REVOKE`) | Medium | Partly |
...
### A2 — `SYS_CAP_GRANT` bypasses the locked cap-write discipline  *(Medium)*

**Where:** `h_cap_grant` in `src/kernel/syscall.c`.

```c
capability_t granted = *src;                 /* full rights, incl. REVOKE */
...
cspace store." `h_cap_grant` violates three parts of it:

1. **No `dest_slot >= KERNEL_RESERVED_CAPS` floor** — `dest_slot` is only bounded
   `< CNODE_SIZE`, so a supervisor can overwrite a child's reserved slots 0–3.
2. **No `caps_in_use` increment** — the granted cap is invisible to
   `MAX_CAPS_PER_TASK`; the ceiling can be exceeded, and a later revocation's
   saturating decrement then desyncs the counter.
3. **No rights reduction** — it copies the source's full rights (see A1).

**Fix (Track 1):** route grant through a locked helper mirroring `cap_transfer` /
`cap_install_endpoint` (reserved-slot floor + was-null check + `caps_in_use`), and
add an optional `new_rights` mask so delegation can *reduce* authority.
