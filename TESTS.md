...
| `make smoke-aspace` | `ASPACE_SELFTEST: PASS` (rebuilding a task slot returns every page the previous occupant held; asserts on the pool count, so a partial reclaim still fails) |
