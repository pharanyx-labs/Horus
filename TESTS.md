...
| `smoke-fb-console-server` | **The end of the chain.** The kernel parses the tag, maps a window and draws the boot log; then `console_server` learns the geometry (`SYS_FB_INFO`), maps the framebuffer from the platform device's own MMIO range, takes the console and paints the login prompt. Before this, a machine whose only display is a framebuffer reached `CONSOLE_SELFTEST: FAIL vga`, audibly, but with **no shell**. Reads **pixels** over QMP, because serial cannot show whether ring 3 drew anything: text near the top **and** the cleared band blank, the second being evidence of the clear rather than of nothing having happened. Bands are measured on both builds rather than guessed, a band the kernel's own grid never reaches separates nothing, which is what the first version of this check chose. The band moved from `y 200-400` to **`y 352-512`** on 2026-09-10 and the reason is worth keeping: it was measured when the login banner was a four-line box, which left **one row** of margin beneath the session, so the neofetch-style banner (fifteen lines to the prompt, deepest ink `y 299`) put 1281 pixels inside a band asserted to be empty. That is a red gate not describing a defect, and at one row it would equally have reddened for a kernel line added before the handover. Re-measured on both builds: `y 352-512` is **0** cleared against **9116** not cleared, so the assertion is the same one with 3.3 rows of margin under the session and nearly three times the ink on the broken side. |
| `smoke-fb-console-server-control` | **Control arm.** `console_server` never asks what the display is, maps a text window that is not there, fails its round trip and parks. Asserts the signature positively: `CONSOLE_SELFTEST: FAIL vga` **and** the kernel's boot log still on screen. Both arms read the **same** band, declared once as `CLEARED_BAND` in `tools/fb_console_test.sh`, so the gate and its control cannot drift onto different bands and quietly stop being a pair. `y 352-512` = **9024** non-black pixels here against **0** when ring 3 clears (measured 2026-09-10; it was `y 200-400` = 3217 against 0, measured 2026-09-08, until the banner grew past it). |
