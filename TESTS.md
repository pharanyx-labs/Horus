...
| `smoke-meta-crash` | **Arm A of `docs/design/meta-cache-merkle.md`**, built before the bounded metadata cache it will guard. Two boots on one image: boot 1 writes 64 distinct blocks — a working set larger than any cache we would configure, so eviction is forced rather than hoped for — and crashes with the last block committed but not applied; boot 2 replays the journal and requires every block back **byte-for-byte**. Byte `i` of block `b` is a function of both, so a block served from the wrong offset looks like a *different block* rather than like zeros, and the marker names which byte diverged. The eviction-count assertion is **deferred, not omitted**: today's full mirror evicts nothing, so it is reported (`evictions=unavailable`) rather than asserted — a check that cannot fail reads as coverage while testing nothing. |
| `smoke-meta-crash-control` | Control arm. `META_CRASH_DROP_ONE=1` loses one block's crypto metadata as an eviction without write-back would: `METACACHE: FAIL block 31 lost after eviction`. **Its first version could not reproduce**, and the reason is recorded in the design doc — skipping one `flush_meta_block` changes nothing, because all 64 blocks share one meta block and the next flush rewrites it from the complete in-RAM mirror. A full mirror is *self-healing* against a lost write; that is the property stage 2 removes, so the arm must clear the in-RAM entry too. |
...
`.github/invariants.yml` holds exemptions only, and is currently **empty**: all 66 properties
