...
**This table is checked by CI.** `tools/check_invariants.py` (required job `invariants`,
roadmap 4.12 / **[F-4.1]**) parses the rows below and fails the build if a property has no
witness that resolves to a real `make` target or CI job, if a witness names a target that does
not exist or that nothing runs, or if a control-arm flag named here is missing from
`DEFECT_FLAGS` and so would go unstamped at boot. The table **is** the registry — there is no
parallel `invariants.yaml`, because a second copy of these claims would drift from this one.
