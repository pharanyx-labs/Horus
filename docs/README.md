...
| [`design/shared-libc.md`](design/shared-libc.md) | Roadmap 2.5 in the shipped system: who holds the shared libc, how a program links it by name in a ring-3 linker, the per-slot private data, and the seal that makes the resolved table read-only: **design only, nothing built**; its decisions are taken |
