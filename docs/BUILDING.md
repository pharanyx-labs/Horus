...
| `KSTACK0_SHARED_PARK=1` | Restores the pre-2026-08-17 park target: when a task dies with nothing else runnable, all three fallbacks in `idt.c` resume the CPU on `tasks[0].kernel_stack_top` — one stack shared by every CPU that takes the path (**[G-8]**, second path). | `make smoke-kstack-park-control`, which requires at least one park stack to come back used by **more than one CPU** — within `KSTACK_PARK_CONTROL_BOOTS` (8) boots, because whether two CPUs park in the same boot is a property of the schedule (measured 9 of 12, and 10 of 12 on `main`). Either the duplicated `PARKTRACE` or the kernel's own collision PANIC counts; the PANIC halts the machine, so on those boots the second trace line never prints |
