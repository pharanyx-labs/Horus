...
Log in as **`root`** / **`rootpass`**, or **`user`** / **`password`** for an unprivileged
session (`users_init` in `src/kernel/kusers.c` is the truth; this line said `horus` until
2026-09-01, which was never a password the kernel accepted). Those are the compiled-in
defaults of a machine with no volume. **An installed disk has neither** -- its account table
lives on the volume, and it carries the two accounts the installer asked you to name, with the
passwords you chose. The compiled-in `user` is deleted during the install, because a fresh
volume's table is seeded from these defaults and would otherwise ship a login whose password is
printed here.
...
type `FORMAT`, then to choose a **root password** twice, then to name an **everyday account**
and give it a password twice.

That root password seals the volume's encryption key *and* becomes the password on the root
account -- different mechanisms with different salts, and a login needs the same string to
satisfy both, which is why it is asked once and applied twice. The second account gets a **key
slot of its own** (`SECURITY.md` **S61**, **S76**), so **either** password opens the disk at
boot; without one it could log in only on a machine somebody else had already unlocked, which
was `docs/LIMITATIONS.md` 2.6b. The two passwords must differ, and the installer deletes the
compiled-in `user` account before creating yours at the same uid.

Every run after that finds a recognised volume, skips the installer, and goes straight to a
login prompt. **Log in with either of the passwords you chose during the install**, not the
compiled-in ones: the account table lives on the volume you sealed. `make run-ata-wipe` is the
...
| `KDIAG_LEGACY_COM1=1` | The pre-2026-09-03 kernel reporter: markers go to the shared console UART and nowhere else, so ring-3 output can cut one in half between two characters (**S81**, `docs/LIMITATIONS.md` 2.6c). | `make smoke-kdiag-legacy-control`, which requires `make smoke-kdiag` to go **red** |
| `KDIAG_PORTS_GRANTABLE=1` | Declares `0x3E8` -- the kernel's diagnostic channel -- among the platform device's grantable ports, so `console_server`'s existing `SYS_IOPORT_GRANT` covers it and ring 3 writes into the kernel's own channel. Reaches **both rings** under one flag, because the claim it falsifies spans both. | `make smoke-kdiag-grant-control` (200 ring-3 sentinels on COM3, no #GP); the base direction is `make smoke-kdiag-ioport` |
| `KDIAG_SPLIT_WIDEN=1` | **Not a defect** -- the window widener: each character of a kernel marker waits for the **AP timer tick** to advance, so a marker spans ~0.5 s of *guest* time on any host and a concurrently-printing task lands inside it. Set in **both** arms of the pair. `KDIAG_WIDEN_SPINS` (200000000) is the wedge guard on that wait, not the window. A spin-count widener was tried first and measured the **host**: 6 boots in 6 locally, 0 in 1 on CI, where the same workload emits 134 ring-3 lines against ~6000 locally. | both arms of `make smoke-kdiag` / `smoke-kdiag-split-control` |
| `KDIAG_NOISE=1` | **Not a defect** -- the second writer. `init` talks to the console forever instead of launching a shell, because the ordinary session goes silent almost immediately (the shell blocks on input, `console_server` blocks serving it) and a probe firing on a tick count either lands in that fraction of a second or does not. Ring-3 only, so it applies to `USERSPACE_CFLAGS`. | every `smoke-kdiag*` arm |
| `KDIAG_PROBE=1` | **Not a defect** -- the marker these gates are about: a survivable kernel report emitted `KDIAG_PROBE_COUNT` (8) times, every `KDIAG_PROBE_EVERY` (20) ticks, after the console handover. `KFAULT_INJECT` cannot serve: its fault kills `console_server`, so no ring-3 writer remains to interleave with. | every `smoke-kdiag*` arm |
| `KDIAG_RING3_PROBE=1` | **Not a defect** -- makes `console_server` attempt a write to `0x3E8`. Set in **both** directions of the authority pair; only the kernel's port declaration moves, which is what separates *ring 3 did not write the channel* from *ring 3 cannot*. Ring-3 only. | `make smoke-kdiag-ioport` (a #GP) and `make smoke-kdiag-grant-control` (it lands) |
...
| `CLAIM_AUDIT_NO_REREAD=1` | Restores the pre-2026-09-02 claim auditor: accuse on the strength of two sightings alone, without asking whether the mismatch is **still there**. The report says *"persisted across two audits"*, and the state behind it is a single file-scope `(task, cpu)` pair with **no time component** -- audit A arms it and audit B, microseconds later on another CPU, sees the same in-flight switch and panics. Both audits run under `sched_raw_lock`, which serialises them rather than excluding the window they are observing. Measured with `CLAIM_IMP_TRACE`: on every audit panic ever captured, a fresh read taken microseconds later agreed with the claim. | `make smoke-claim-reread-control` (`CLAIMREREAD_SELFTEST: FAIL a resolved mismatch was still accused` present); `make smoke-claim-reread` must go red under the same flag. Deterministic -- the predicate is a pure function of three variables, staged directly. **The re-read costs no detection**, and the gate asserts that in the same run: a leaked claim is permanent, so it cannot resolve between the sighting and the re-read, and the test requires a live leak to still be reported |
| `KSTACK_COLLIDE_IMPERSONATED=1` | Restores the pre-2026-09-02 identity in the **G-8 collision detector**: `get_current_task()`, i.e. `percpu_current_task[]`, which `scheduler.c`'s own header calls *"deliberately lying"* for the whole of every impersonation window. A CPU impersonating a peer that blocked on another core -- and has not been unwound off its stack yet -- then asks "is anyone else on that task's stack", gets yes, and reports **two CPUs on one kernel stack** while neither CPU is on the other's. A **false positive in an S20 witness**, and the same blind spot `percpu_real_task[]` was introduced to close for the claim auditor, reached through a different caller. | `make smoke-kstack-imp-control` (`KSTACKIMP_SELFTEST: FAIL an impersonating cpu was accused of sharing a kernel stack` present); `make smoke-kstack-imp` must go red under the same flag. Deterministic -- the selftest stages the three variables directly, because the natural event is ~0.3% of boots (**[G-12]**) and that is no basis for a gate |
| `CLAIM_IMP_TRACE=1` | Not a defect, an instrument, and a narrower one than `CLAIM_TRACE`. It changes no behaviour, runs only on the claim auditor's mismatch path, and re-reads the accused CPU's impersonation state at the instant of the accusation: the depth either side of the reads (`d1`/`d2`, so `torn=1` means the CPU changed impersonation state *during* the probe), what each branch of `sched_running_on()` would have returned (`cur`/`real`), and a fresh evaluation (`seen2`). **[G-12]** is stuck on whether the auditor is telling the truth -- `sched_running_on()` reads the impersonation depth and then the current task, and `sched_impersonate_enter()` takes no scheduler lock -- and `seen2 == t` says the mismatch is gone on re-read while `seen2 != t` with `torn=0` says the leak is real. Those two want completely different fixes and are indistinguishable in the report as it stands. | Used for measurement; not a gate. Requires `SCHED_INVARIANTS=1` for the auditor it instruments. |
...
| `SYSCALL_PTR_TRUNC32=1` | Restores the pre-2026-08-20 `sys_dmesg` / `sys_audit_digest` wrappers, which passed their buffer as `(uint32_t)(unsigned long)ptr`: so the kernel received the low 32 bits of an address the caller never named and resolved it in the caller's own address space (issue #176). Reaches userspace only: `USERSPACE_CFLAGS` is assigned with `=`, and the flag must be applied at top level after that and outside any other flag's `ifeq`, which is where it was first written and silently never fired. | `make smoke-klog-forge-abi-control`, which requires `KLOGTEST: FAIL setup` to be **present** (3 boots in 3, `rc=-14`), against `make smoke-klog-forge`, whose probe reads the log into a static, hence above-4-GiB; buffer. Since 2026-09-01 it also reaches `sys_audit_digest` and `sys_read_audit`: `make smoke-auditprobe-control` requires `AUDITPROBE: FAIL digest-into-a-static-buffer`, 3 boots in 3, and the transcript shows the **signature** rather than just a failure — the probe's stack calls still succeed and only its static ones do not. `make smoke-auditprobe` must go red under it. It reaches only wrappers spelled `SYSCALL_UPTR()`, which is 5 of the 52 pointer arguments `tools/check_syscall_abi.py` covers; the rest use an equivalent explicit 64-bit cast and sit outside this arm — so this arm's reach is a property of how each wrapper is spelled, not of the check |
...
| `META_CACHE_TINY=1` | **Not a defect — a window widener.** Drops the bounded metadata cache from 32 lines to 2, so a working set of a few hundred blocks provably exceeds it and eviction is forced rather than hoped for. Set in **both** arms of the crash gates; that is what makes the pair a measurement (the `KSTACK_RACE_WIDEN` pattern). Stamped into `DEFECT FLAGS` deliberately: a number measured on a two-line cache is not a number measured on the shipped one. | every arm of `make smoke-meta-crash` |
| `META_CACHE_NO_WRITEBACK=1` | Removes the metadata cache's write-back entirely: `journal_commit` does not flush dirty lines and eviction drops them, so a metadata update a committed transaction promised never reaches the disk (failure modes E1 and E4 in `docs/design/meta-cache-merkle.md`, **S65**). | `make smoke-meta-crash-control` (`METACACHE: FAIL block 0 lost after eviction`); `make smoke-meta-crash` must go red under it |
| `META_CACHE_WB_OUTSIDE_TXN=1` | Keeps the write-back and moves it **past the end of** `journal_commit`, where `do_block_write` goes straight home instead of into the journal. Every value written is correct; what is gone is the atomicity between a block's nonce and its ciphertext (failure mode **E2**). | `make smoke-meta-crash-txn-control` (`METACACHE: FAIL block 399 lost after eviction` — the block the crash committed, a *different* block from the arm above, which is what shows the two rules fail independently); `make smoke-meta-crash` must go red under it |
| `META_CACHE_EVICT_NOWB=1` | Removes **only** the eviction write-back, leaving the commit flush in place. **No gate, deliberately:** measured 2026-08-31, `make smoke-meta-crash` PASSES under it, because with the commit flush present no line is ever dirty by the time anything can evict it — every crash-gate boot prints `dirty=0` beside its eviction count, so this is a measurement rather than an argument. A control arm that cannot fail cannot gate; the same call `SPAWN_STAGE_UNSERIALISED` got. Kept for the day a transaction dirties more lines than the cache holds, which is the only way that path becomes reachable. | no gate — see the reason |
| `MERKLE_NODE_TRUST_CACHED=1` | Sets a Merkle node line's `verified` flag where the line is **filled** rather than where it is checked, so residency in the node cache becomes the trust criterion instead of a path from the root (failure mode R1, **S66**). | `make smoke-merkle-replay-control` (`MERKLE: FAIL stale node accepted - subtree served a rolled-back block`, 6 of 6 blocks served); `make smoke-merkle-replay` must go red under it |
| `MERKLE_SKIP_PARENT_BIND=1` | Checks a metadata block's hash against the level-0 node that records it and does **not** place that node under the root. Forgeries and swaps are still refused — measured, by restoring the block without its node, which this build still refuses 6 of 6 — so what is gone is only the question of whether the state is the *current* one, which is the whole difference between a Merkle tree and a set of independent MACs. | `make smoke-merkle-parent-bind-control`; `make smoke-merkle-replay` must go red under it. A second arm because the witness returns at its first failure and the arm above never reaches this rule |
| `FSCK_SHALLOW_REFS=1` | Restores the pre-2026-08-31 `storage_fsck_pass` reference walk: `direct[]` and the single-indirect block, and nothing below. Every block reachable only through the double-indirect tree is then reclaimed at **every unlock** and handed to the next caller that allocates (**S67**). | `make smoke-fsck-refs-control` (`FSCKREF: FAIL fsck freed 4 of 4 blocks of a live file`); `make smoke-fsck-refs` must go red under it |
| `ROLLBACK_ANCHOR_IGNORE=1` | Drops the comparison of `sb.rollback_gen` against the TPM NV counter, so a volume older than the machine mounts and serves the state it held when the disk was imaged (**S70**). Everything else still runs — the volume is still authentic, the Merkle tree still verifies, every byte was genuinely produced by this volume; what is gone is the only question the disk cannot answer about itself, which is *when*. | `make smoke-rollback-control`, whose boot 3 reports `found era 1` — the state the machine had already left, where a current volume holds era 2; `make smoke-rollback` must go red under the same flag |
| `ATA_READY_ERR_ONLY=1` | Restores the pre-2026-09-01 transfer rule: ERR alone decides. That accepts a status with BSY still set (the wait timed out) or DRQ never asserted (the drive has no data), so the driver transfers 256 words against a drive that said nothing and reports success (**S69**). | `make smoke-ata-ready-control`, which requires the predicate to accept **128 of 256** statuses, the first being `0x00`; `make smoke-ata-ready` must go red under the same flag |
| `ALLOC_NO_HINT=1` | Restores the pre-2026-09-01 block allocator: every scan starts at data-bitmap block 0, so a volume whose first N bitmap blocks are full costs N+1 reads **per allocation**. Measured at **512 bitmap reads for 32 allocations against 47** with the hint, on a 2 GiB volume with 15 of 16 bitmap blocks full. | `make smoke-alloc-hint-control` (`ALLOCHINT: FAIL every allocation rescans the bitmap from the start`, with the count printed beside it); `make smoke-alloc-hint` must go red under the same flag |
| `STORAGE_MOUNT_ANY_SIZE=1` | Drops the check that a superblock's `total_blocks` fits the device it is on, so a volume formatted on a larger disk mounts on a smaller one — part of a filesystem served as whole, with the data region's tail simply not there (**S68**). | `make smoke-fs-shrink-control`, whose marker is that the truncated volume **mounted** rather than that a refusal was absent; `make smoke-fs-shrink` must go red under the same flag |
| `VDISK_TOTAL_UNBOUNDED=1` | Restores the pre-2026-08-31 RAM vdisk: `total_blocks = BLOCKS_PER_DISK` over a `VDISK_BLOCKS`-sized reservation, and no second bound against the backing store. Blocks past the reservation are accepted and land in the free page pool (**S64**). | `make smoke-vdisk-bound-control`, which requires the pattern to be READ BACK out of the pool rather than requiring a refusal to be absent; `make smoke-vdisk-bound` must go red under the same flag |
| `TUI_NO_DAMAGE_DIFF=1` | `tui_flush` repaints every cell instead of only the changed ones. The screen is identical either way, which is exactly why this needs an arm: damage diffing is what makes a full-screen UI usable over a serial line, and nothing you can see distinguishes it from a repaint. | `make smoke-tui-diff-control`, which requires `TUITEST: FAIL a one-cell change repainted the screen` to be **present**; `make smoke-tui` must go red under it |
| `TUI_CLAMP_OFF=1` | Removes the single bounds check every TUI drawing call funnels through, so a write past the last row or column lands in the cell buffers. The memory-safety half of the pair. | `make smoke-tui-clamp-control`, which requires `TUITEST: FAIL an out-of-range write reached the buffer` to be **present**; `make smoke-tui` must go red under it |
| `INSTALLER_NO_CONFIRM=1` | The installer reads the typed confirmation word and then does **not compare** it, so the install proceeds whatever was typed -- including nothing (**S73**). Only the comparison goes: removing the screen would also remove the keystrokes the harness sends, and the two arms would then be driving different conversations instead of differing in one decision. | `make smoke-installer-refuse-control`, which types the **same wrong word** as the base arm and then requires the install to continue to `INSTALLER: formatting`; `make smoke-installer-refuse` must go red under the same flag (measured 2026-09-01: exit 2, timeout waiting for `INSTALLER: nothing was written`). **The arm drives the install to the format rather than stopping at the bypass**, and its first version did not -- it asserted `formatting` immediately after the wrong word and timed out while the installer sat at the password prompt waiting for input the refuse-mode harness never sends. The defect had reproduced perfectly and the arm reported a timeout: the right property asserted at the wrong point in the conversation |
| `STORE_LOCKED_UNCHECKED=1` | The pre-2026-09-01 object-store handlers, which tested `mfs->mounted` and not `mfs->unlocked` (**S74**). A sealed volume -- mounted and locked, the normal state of an installed machine between power-on and the password prompt -- then answered `SYS_FS_STAT` with a real inode and accepted `SYS_FS_INODE_ALLOC` / `SET_META` / `SET_SIZE` / `INODE_LINK` / `INODE_FREE` against its **plaintext inode table**. The AEAD caught the same omission for file DATA and nothing caught it for metadata, which is why the half without a key requirement enforced nothing and had no reason to look wrong. **The raw block API is not affected and not changed**: `h_block_read`/`h_block_write` sit below the volume abstraction on purpose. | `make smoke-installer-provision-control`, which requires `FS_STORE: open at startup; provisioning now` on a boot whose volume is sealed -- a **positive** marker naming the branch fs_server took, not an absent one -- and then requires the empty `/bin` that answer leaves behind for good; `make smoke-installer-provision` must go red under it (measured 2026-09-01: exit 2, `/bin is empty on the installed disk`). The gate powers the machine off **at the login prompt** on the install boot, because the copy into `/bin` runs when the volume unlocks and finishing it is the next boot's job -- the only boot on which this is observable, and one every real install passes through |
| `PASSWD_TARGET_IGNORED=1` | The pre-2026-09-01 shell `passwd`, which MATCHED an argument (`strncmp(cmd, "passwd ", 7)`) and then dropped it: every call went to `sys_getuid()` (**S75**). A root operator who created an account and typed `passwd 1001` to give it a password got `password changed` -- true of a different account than the one named -- and silently changed their **own**. Not cosmetic: `do_passwd` re-wraps the volume key when `target_uid == my_uid`, so the string typed for somebody else's account became the volume's key-slot password. **Userspace only**, so the `-D` is applied at top level after `USERSPACE_CFLAGS` is assigned with `=` and outside any other flag's `ifeq` -- the trap `SYSCALL_PTR_TRUNC32` records. The arm restores the **mis-targeting** and keeps the fix's reply, which now names the uid it wrote; its real assertions are the two logins, which do not depend on that message at all. | `make smoke-passwd-target-control`, two boots on one image: boot 1 requires `password changed for uid 0` from a `passwd 1001` and then a **successful** `root` login with the OTHER account's password, and boot 2 requires the volume itself to open with it -- every assertion a login that SUCCEEDS, because a refusal is satisfied by a boot that never reached a prompt. `make smoke-passwd-target` must go red under it (measured 2026-09-01: exit 2, `root's own password was changed by passwd <uid>`) |
| `PASSWD_NO_KEYSLOT=1` | The pre-2026-09-02 `do_passwd`: an administrator sets another account's password and **no key slot is granted** (**S76**), so that password opens the account and not the volume. `h_auth` unlocks before it consults the account table, so such an account cannot be the **first** login after a power cycle -- it works perfectly on a machine somebody else has already opened, which is `docs/LIMITATIONS.md` 2.6b and why it went unnoticed for so long. The hash is still set and the call still reports success: the defect was never a failure, it was a success with the invisible half left out. | `make smoke-installer-accounts-control`, which installs two accounts, powers the machine off, and requires the everyday account to be **refused as the first login** -- then requires root to log in, and then the **same user** in the same boot. That third step is what makes the pair mean anything: it separates "the account is broken" from "the account has no key slot". `make smoke-installer-accounts` must go red under it (measured 2026-09-02: exit 2, `2.6b is not closed: the user cannot be the first login after a power cycle`) |
| `SHELL_FS_ERR_FLAT=1` | The pre-2026-09-02 shell, which had the `fs_server`'s rc in its hand and printed a guess: one sentence for every outcome of every file command, `failed (name exists or server not running)`, naming two causes and not the one that fires most often. A standard user's `mkdir` in the root-owned 0755 `/` is `SYS_ERR_PERM` out of `perm_ok(&st, cuid, cgid, P_W)` -- the reference monitor working -- reported as a name clash or a dead server. **Userspace only**, so the `-D` that matters is applied to `USERSPACE_CFLAGS` at top level, outside any other flag's `ifeq`; the `CFLAGS`/`ASFLAGS` pair beside it exists to stamp `DEFECT FLAGS` and churn `.build-flags`. | `make smoke-session-fs-err-control`, which drives the **same commands** as the base arm with `SESSION_FS_ERR_FLAT=1` and requires the flattened sentence back; `make smoke-session` must go red under the same flag (measured 2026-09-02: exit 2, timeout waiting for `mkdir: name exists`, with the flattened sentence in the transcript). The target runs `make clean` first and that is load-bearing rather than tidy: `userspace/%.o` has **no `.build-flags` prerequisite**, so a userspace-only `-D` does not force a rebuild by itself and an arm over stale objects passes with the defect absent |
| `USERLIST_UNGATED=1` | Removes `SYS_USERLIST`'s `CAP_USER` test, so any ring-3 task reads the account table (**S78**). Kernel flag: the gate is in `do_userlist`, beside the one `useradd`/`userdel`/`passwd` share. | `make smoke-captest-userlist-control` (`CAPTEST: FAIL userlist-without-cap-user` present); `make smoke-captest` must go red under the same flag. The ungated call **succeeds** rather than merely getting further -- a booted system always has accounts, so captest reads `root` back -- which is what a refusal test needs: one whose ungated form also failed would pass under the defect and witness nothing |
| `HOME_DIR_ROOT_OWNED=1` | `fs_server` creates `/home/<name>` and then does **not** give it away, so the directory exists and the account cannot write in it (**S78**). Reaches the pre-2026-09-02 state -- an account with nowhere writable -- by a route the account can observe, where before there was simply nothing in `/home` at all. | `make smoke-session-home-control`, which requires **both** halves to fail: the account cannot `stat` its own home (refused for `permission denied`, since a 0700 root-owned directory gives `other` nothing) and cannot create in it; `make smoke-session` must go red under the same flag. Two checks because ownership and a usable mode are different claims, and either alone is satisfied by the wrong system |
| `FS_CHMOD_ANY_OWNER=1` | Removes `FS_OP_CHMOD`'s owner-or-root test in `fs_server`, so any caller sets the mode of any file (**S77**). | `make smoke-session-chmod-control`, which requires a standard user's `chmod 777` of a **root-owned** file to succeed -- and requires the chown refusal beside it to still hold, which is what shows the two rules are independent; `make smoke-session` must go red under the same flag |
| `FS_CHOWN_ANY_UID=1` | Removes `FS_OP_CHOWN`'s root-only test, so any caller gives a file to any uid -- in practice, takes one (**S77**). | `make smoke-session-chown-control`, which requires a standard user to take ownership of a root-owned file, with the chmod refusal beside it still holding; `make smoke-session` must go red under the same flag. The two arms deliberately target **different files**: under this arm the user owns `note` afterwards, so a shared target would satisfy the chmod arm for the wrong reason |
| `TUI_INPUT_ECHO_SECRET=1` | Drops the mask in `tui_input`, so a password field paints what was typed. **The returned value is identical either way**, which is the whole difficulty: neither the caller nor a test that inspects the buffer can tell a masked field from an echoing one, and the disclosure is to whoever is looking at the screen or the serial capture. Only the cells distinguish them. | `make smoke-tui-mask-control`, which requires `TUITEST: FAIL a masked field showed its characters` to be **present**; `make smoke-tui` must go red under it. The witness reads the back buffer rather than a return value, and asserts "it drew stars" and "it did not draw the secret" as two checks -- those are the same sentence only while the alphabet excludes `*` |
| `TUI_INPUT_UNBOUNDED=1` | Drops the `cap` bound in `tui_input` and **keeps** the visible-width bound, so a caller that passed a small buffer and a wide field is written past the end of it. Keeping the second bound is deliberate: removing both would reproduce an unbounded write, which no realistic mistake makes, where the mistake this guards against is the ordinary one of trusting a single bound. | `make smoke-tui-bound-control`, which requires `TUITEST: FAIL an input overran the buffer it was given` to be **present**; `make smoke-tui` must go red under it. The guard region lives **inside the same array** as the buffer under test -- adjacency between two separate arrays is the linker's business, not the language's |
| `TUI_MENU_UNCLAMPED=1` | Lets a menu's selection run past either end of its item list. It corrupts nothing in the TUI -- every cell it paints is clamped by `tui_putc` regardless, so the **screen is identical** -- it corrupts the program using it, which indexes `items[*sel]` on return. For an installer that array is the list of disks it is about to destroy one of. | `make smoke-tui-menu-control`, which requires `TUITEST: FAIL a menu selected past its last item` to be **present**; `make smoke-tui` must go red under it. The marker is the returned INDEX rather than anything drawn, and both ends are checked -- a single-direction arm would miss a clamp lost only at the top |
| `TUI_ACS_NO_RESTORE=1` | Stops `tui_flush` **emitting** the shift back from the DEC Special Graphics charset, while leaving the bookkeeping that says it did. Line drawing works by telling the terminal to reinterpret ordinary letters (`q` is a horizontal rule, `x` a vertical one), so a flush that ends in that charset renders every subsequent byte as a glyph -- and it corrupts nothing this library owns, because every cell it draws carries its own charset decision. What it corrupts is everything that is **not** the library: the task's own marker lines, and the login prompt after `tui_end`. The bookkeeping deliberately still runs: dropping it too would leave `ographics` never seeing a transition, so no box would be drawn at all, and the arm would fail by producing no line drawing rather than by failing to restore -- a different defect wearing this one's name. | `make smoke-tui-acs-control`, which requires `TUITEST: FAIL a flush left the terminal in the line-drawing charset` to be **present**; `make smoke-tui` must go red under it. **The witness reads the emitted stream, and it has to**: no cell, no return value and no byte COUNT shows this -- the count goes DOWN, so a count-based check would read the defect as an improvement |
| `TUI_WRAP_NO_BREAK=1` | Lets `tui_wrap` emit a word longer than its column whole instead of breaking it at the column, so the text runs out of its column and across whatever is beside it. This is the ordinary wrap bug rather than an invented one: the space-breaking path is the one that gets tested and the no-space case is the one that gets forgotten. It stays bounded by the **screen** -- `tui_putc` still discards past the last column -- for the reason `TUI_INPUT_UNBOUNDED` keeps its width bound: the mistake being reproduced is trusting one bound, not writing off the end of memory. | `make smoke-tui-wrap-control`, which requires `TUITEST: FAIL a wrapped word ran out of its column` to be **present**; `make smoke-tui` must go red under it. The witness is a cell past `col + width` on the same row, which on every installer screen is where the frame's right-hand edge lives |
| `TUI_NO_INVALIDATE=1` | Makes `tui_invalidate` a no-op, so the library keeps believing a screen something else has written over. The damage diff rests on the assumption that nothing writes to this terminal except `tui_flush`, and that is **false for every program here that also emits markers**: those are cooked `CON_OP_WRITE` requests to the same UART, landing at wherever the terminal's cursor is. The library cannot see a write it did not make, so `front` still says those cells are correct and every later flush skips them -- the foreign text stays put through that screen and every screen after it. **This was the shipped behaviour until 2026-09-06**, and it was found by rendering the installer's own serial stream through a VT emulator, as `*******INSTALLER: waiting on the user password again` across a live install's password row. | `make smoke-tui-invalidate-control`, which requires `TUITEST: FAIL an invalidated screen was not repainted` to be **present**; `make smoke-tui` must go red under it. **No installer gate catches this and none can**: measured 2026-09-06 over one install rendered both ways, `make smoke-installer` **passes under the flag and without it**, with 16 rows of marker text drawn into the frame against 0 -- the markers are on the wire either way, which is all those gates assert on |
| `STORAGE_SINGLE_DEVICE=1` | Restores the pre-2026-09-06 ATA probe: only the primary master is ever looked at. A machine with two disks then reports **one**, and the second is invisible to the survey, to the mount policy that decides which volume this machine boots, and to an installer asking which disk to erase. It is the PROBE that is cut rather than the enumeration, because that is where the old code stopped -- everything downstream already walks a list, and cutting the list instead would leave the driver finding a device nothing reports, which is a different and weaker defect since the survey would still be right about what the driver knew. | `make smoke-storage-survey-single-control`, which boots **two** blank disks and requires `INIT_STORAGE: 1 persistent device(s)` to be **present**; `make smoke-storage-survey` must go red under it. **Both arms assert a COUNT, not a presence**: "the survey found a disk" is true in both arms and on a one-disk machine, so it distinguishes nothing, and an arm asserting the second disk's ABSENCE would be satisfied by a boot that never surveyed at all |
| `STORAGE_DEVICE_INDEX_CLAMP=1` | Answers an out-of-range device index with the LAST device instead of refusing it. **Nothing faults and nothing overruns**: the caller gets a complete, well-formed survey OF THE WRONG DISK, and the one caller of that survey is a program deciding which disk to erase. This is the shape a bounds check takes when somebody makes it "forgiving", and it is why the property is the refusal rather than the absence of a crash. | `make smoke-storage-device-clamp-control`, which requires `INIT_STORAGE: FAIL an index past the last device was answered` to be **present**; `make smoke-storage-survey` must go red under it. The marker is init's own FAIL line rather than a fault, because "the call succeeded" is exactly what the defect looks like from ring 3 |
| `STORAGE_FORMAT_TARGET_IGNORED=1` | Validates the device index handed to `SYS_STORAGE_FORMAT` and then **discards** it, so the format lands on whatever device the machine nominated at boot rather than the one the operator chose (**S83**). Every check still passes, every return code is still 0, and the disk that gets erased is a different one from the disk that was named on the screen. | `make smoke-installer-target-control`, which drives a two-disk install onto disk 1 and requires the run to fail with `the volume is not on the disk that was chosen`; `make smoke-installer-target` must go red under it. **The arm greps the harness's own output, not the guest's serial**: that verdict is a `SessionFail` raised in the test and printed on stderr, and it never appears on the wire -- grepping the serial log for it scored a real reproduction as "failed for some other reason" |
...
| `STORAGE_FORMAT_UNGATED=1` | Removes the dispatch-table row in front of `SYS_STORAGE_FORMAT` **entirely** -- no slot, no rights floor, no type -- so any ring-3 task may authorise a format of the attached disk (**S72**). The whole row goes rather than one half of it: with the type in place an empty slot is refused by emptiness, and with the slot in place a wrong type is refused by type, so an arm against either half alone would pass for a reason that is not the one being measured. Non-destructive in the arm's own boot -- `captest` runs on the ephemeral vdisk, which is already mounted and unlocked, so `storage_unlock` is idempotent and returns 0 -- and that is exactly what makes the refusal a measurement: a refusal test whose ungated path ALSO failed would pass under the defect and witness nothing. | `make smoke-captest-storage-format-control`, which requires `CAPTEST: FAIL storage-format-without-cap-storage-format` to be **present**; `make smoke-captest` must go red under the same flag (measured 2026-09-01: exit 2, `SMOKE FAIL: saw fail marker`). The positive direction is `make smoke-storage-survey`, two boots of one kernel with and without a disk attached |
...
| `AUDIT_ABI_LEGACY=1` | The pre-2026-09-01 `SYS_READ_AUDIT` ABI: ring 3 declares a 72-byte `struct audit_record` and `h_read_audit` copies the kernel's 256-byte internal event at the kernel's stride into it, so every field is read from the wrong offset **and** the copy runs 184 bytes past the array per record (**S71**). **Both rings under one flag** — the defect is the disagreement, so an arm that changed one side would reintroduce something that never existed. | `make smoke-auditprobe-abi-control`, which requires `AUDITPROBE: FAIL read-audit-wrote-past-the-array` — the overrun rather than the misread field, because that half no return code can report — **3 boots in 3**, with `read-audit-did-not-return-our-own-event` printed beside it; `make smoke-auditprobe` must go red under the same flag |
...
| `IMAGE_HDR_WRITER_SKEW=1` | Makes `tools/mkheadered` emit `name` four bytes further into the **same** 44-byte `.bin` header, as if a `uint32_t flags` had been inserted before it on the WRITER side only. That is the one-sided layout change four unconnected copies of the format could not catch (**S80**, `docs/LIMITATIONS.md` 2.18). Deliberately quiet: `magic`, `entry`, `size` and the payload offset all stay correct, so the image **arms successfully** and only the name comes back wrong -- an arm that broke the magic would be caught by any reader, and one that only asked "did it load?" would pass. It reaches the tool through `MKHEADERED_SKEW`, not `CFLAGS`, because `mkheadered` is a host build with its own rule; the arm therefore has to regenerate every `.bin`, which is why its target cleans first. | `make smoke-image-abi-control`, which requires `IMAGE_ABI: FAIL name-mismatch` to be **present** -- that specific marker, so an arm failing at `arm-failed` (a loud break) would not satisfy it; `make smoke-image-abi` must go red under the same flag |
| `LEGACY_SYSCALLS_PRESENT=1` | Restores **six** retired dispatch entries. Four went on 2026-08-23: `SYS_CLEAR` (5), `SYS_SYSINFO` (6), `SYS_DEBUG_EXEC` (7) and `SYS_EXEC_LEGACY` (14), which **creates a task**. Two more went on 2026-09-03: `SYS_EXEC` (19), which drops the caller to ring 3 at an address it picks, and `SYS_RECEIVE_PROGRAM` (27), which arms a program image. All but `SYS_DEBUG_EXEC` were authorised on cspace slot 3 with `SC_ANYTYPE` -- the legacy `CAP_FRAME` every task is born holding, so [C-1]'s decoy and by **S28** not a gate. [H-3] swept that shape and called its three "the last three"; #201 found a fourth, invisible because the entry was written `[14]`; audit 4.1 moved five more to `CAP_UNTYPED`. None of the three enumerated 19 or 27, which is why `tools/check_dispatch_gates.py` now reads the table instead (**S79**). | `make smoke-passwd-probe-legacy-control` requires `PASSWDPROBE: FAIL legacy-exec-spawned-a-task` **present** -- that specific marker, so an arm reddening the probe for another reason does not satisfy it. Two more, one per door added in 2026-09-03: `make smoke-passwd-probe-recv27-control` requires `PASSWDPROBE: FAIL receive-program-reachable-on-the-slot-3-decoy`, and `make smoke-passwd-probe-exec19-control` requires **`SYSCOV 19`** (with `SYSCALL_COVERAGE=1`) -- a kernel-side marker, because `h_run` ends in `drop_to_ring3` and never returns, so the defect destroys any ring-3 reporter. `make smoke-passwd-probe` must go red under the flag |
...
| Target | Asserts |
|---|---|
| `smoke-captest` | Unheld capabilities, post-revoke use, and bad input are all refused (182 checks, read the count off `CAPTEST: PASS <n> checks` on the wire, not from here) |
