...
- **Boot modules are integrity-checked (audit A4 — fixed).** Coreutils/man-page GRUB modules used to become root-owned `/bin` executables trusted only by boot-chain provenance. The kernel now embeds a SHA-256 manifest of exactly the modules it was built to ship and verifies each at boot (`boot_module_verify_all`), refusing any mismatch at the `SYS_BOOT_MODULE_INFO`/`READ` choke point, with a destination allowlist restricting modules to `/bin` and `/usr/share/man`. Gated by `make smoke-modules-tamper`. **Residual:** the manifest is only as trustworthy as the kernel image carrying it, so full closure depends on pinned/attested builds (P4).
