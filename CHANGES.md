...
- **Ring-3 process control** — `SYS_SPAWN` (spawn a named child, hands the caller its `CAP_TCB`), `SYS_EXEC_NAMED` (replace the caller's image in place), `SYS_CAP_GRANT` (delegate a cap into a supervised child), `SYS_KILL` / `SYS_EXIT` (terminate, `CAP_TCB`-gated), `SYS_SIGNAL` (async task-to-task signal, `CAP_TCB`-gated), and `SYS_WAIT` (block until a task exits). Proven by `make smoke-proc`.
...
- **Signals** — fault signals (`SYS_SIGACTION`/`SYS_SIGRETURN`: a ring-3 fault is delivered to a registered handler instead of killing the task) and async task-to-task signals (`SYS_SIGNAL`). Handler address validated in safe Rust; no re-delivery inside a handler; no new authority. Proven by `make smoke-signal` and `make smoke-proc`.
...
- **Filesystem** — a ring-3 `fs_server` over the kernel's encrypted object store (syscalls 56–61), reached over IPC; real `ls`/`cat`/`mkdir`/`rm`/`touch`/redirection. RAM vdisk by default, ATA with `STORAGE_ATA=1`. Proven by `make smoke-fs`. A legacy in-memory capfs (`SYS_FS_*`) also exists.
...
- **Table-driven syscall dispatch** — one descriptor table (numbers 0–66) enforces each syscall's fixed capability at a single choke point; unlisted numbers fail closed; a compile-time assertion pins the table to the syscall number space.
