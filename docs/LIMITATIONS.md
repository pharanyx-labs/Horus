...
**Measured 2026-08-20**, and gated since: **55 of 85** implemented syscalls have their handler
body entered by the three tracked workloads (the scripted ring-3 session, the conformance suite, and the
boot-modules session). The other 30 are listed in `.github/syscall-coverage.yml`, each with a written reason.

This is stated as a limitation rather than a finding because nothing here is known to be
broken. What is known is that a defect in any of those 30 handlers would be invisible in the
...
**Two of the 30 are cheap and worth doing next.** The pipe family and `SYS_STDIO_INFO` are
