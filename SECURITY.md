...
- **Capability revocation over-revokes (audit A1).** The revocation sweep matches an *object/badge/serial equivalence set* rather than a derivation subtree, so revoking a delegated capability can also null the grantor's original and same-`object` peers. It **fails safe** (it removes authority, never grants it — not a privilege escalation), but it violates the intended least-privilege-delegation and transitive-revocation semantics. Being reworked to a proper capability derivation tree (Roadmap Track 1).
- **`SYS_CAP_GRANT` skips the locked cap-write discipline (audit A2)** — no reserved-slot floor, no `caps_in_use` accounting, and it copies full rights including `CAP_RIGHT_REVOKE`. Being routed through the locked mint path with rights reduction (Track 1).
