...
| 108 | `SYS_SHLIB_INFO` | `frame_slot`, `struct shlib_info *`: where the shared library is loaded **this boot**. The base is drawn from the ASLR source, not compiled in, so a program cannot assume it. Requires a `CAP_FRAME` + READ naming one of the library's own **text** frames: the base is the address of code every task executes, and a task's private copy of the library's data page does not qualify. `flags` carries `SHLIB_INFO_DATA_MAPPED` when the kernel mapped the caller's private copy of the library's data at spawn or exec (S106), so the caller maps only the text |
| 121 | `SYS_MEM_SEAL` | `addr`, `len`: make pages of the caller's own image read-only for good (S107). `PAGE_WRITE` and `PAGE_COW` are cleared and the page is marked sealed, so no copy-on-write break, fork or device mapping can make it writable again. Only drops rights, so no capability gates it; bounded to the caller's `[image_base, image_end)`, page-aligned, all or nothing. A write to a sealed page is SIGSEGV. For the ring-3 linker's RELRO |
