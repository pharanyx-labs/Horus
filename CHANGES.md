...
- **Table-driven syscall dispatch** — one descriptor table (numbers 0–74) enforces each syscall's fixed capability at a single choke point; unlisted numbers fail closed; a compile-time assertion pins the table to the syscall number space.
