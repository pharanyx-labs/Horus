...
| `NET_DMA_ADDR_VIRTUAL=1` | `netd` programs its own **virtual** addresses into the queue-address register and every descriptor, instead of the answer from `SYS_DMA_ADDR`. Both are numbers and both look plausible; only one is where the device will go. Every register write still succeeds and the device accepts both queues — it simply never finds a descriptor. | `make smoke-net-dma-control`, which requires `NETTEST: FAIL no-arp-reply` to be **present** (3 boots in 3); `make smoke-net` must go red under the same flag |
...
| Target | Asserts |
|---|---|
| `smoke-captest` | Unheld capabilities, post-revoke use, and bad input are all refused (126 checks — read the count off `CAPTEST: PASS <n> checks` on the wire, not from here) |
