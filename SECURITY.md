...
- **Supply chain / CI:** every change is gated by a pipeline of 26 jobs (25 gating) — `cargo test`, `clippy` with all warnings denied, a kernel + ISO build, an alt-config build matrix (`DEBUG_SHELL=1`, `MINIMAL_SECURE=1`, `SMP=1`), twenty-one headless QEMU self-tests (boot, the kernel W^X leaf sweep, the CR4 protections actually being in CR4, ELF-loader + W^X for both ELF classes, image-base ASLR, preemption, signals, process-control, copy-on-write, notifications, SMP, a scripted ring-3 shell session, and the filesystem/libc suite), a byte-for-byte reproducible-build check, and an advisory security-scan job (Semgrep, Trivy, gitleaks, cppcheck, flawfinder, `cargo-audit`) that also emits a CycloneDX SBOM.
