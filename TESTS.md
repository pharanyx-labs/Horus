...
`.github/invariants.yml` holds exemptions only, and is currently **empty**: all 48 properties
...
| Capability conformance (131 checks — the suite prints its own count as `CAPTEST: PASS <n> checks`; read it from there) | `userspace/captest.c` | `make smoke-captest` |
