...
DEFECT_FLAGS = \
	...
	KFAULT_INJECT KFAULT_LEGACY_PRINTLN KFAULT_RECORD_SELFTEST EXIT_RECORD_KERNEL_RIP EXIT_RECORD_STALE_ON_REUSE PAGE_FREE_UNGUARDED \
...
# defined below.
# KEYSLOT_SELFTEST=1 builds the two-boot key-slot witness: several passwords open
# one volume, and revoking one revokes exactly that one. Also compiles
...
ASFLAGS += -DKEYSLOT_SELFTEST
endif

# KEYSLOT_REMOVE_NOOP=1 is the falsifying arm: storage_keyslot_remove reports
# success and leaves the wrap openable. A count-based check passes under it,
# which is why the witness probes the PASSWORD.
KEYSLOT_REMOVE_NOOP ?= 0
ifeq ($(KEYSLOT_REMOVE_NOOP),1)
CFLAGS  += -DKEYSLOT_REMOVE_NOOP
ASFLAGS += -DKEYSLOT_REMOVE_NOOP
endif
...
# module, for smoke-boot-module-reserve and its control arms only. GRUB places
# modules upward from the end of the kernel image, so a 16 MiB module starting
# anywhere below USER_PHYS_BASE necessarily runs past it, and every module after
# it lands above 16 MiB: where the page pool's base reserves sat until
# 2026-09-19, and where they must now move out of the way of (S96). First in
# BOOT_MODULES on purpose, so the verified executables behind it are pushed up
# too, which is the realistic case. It does not fit the store volume, so
# fs_server skips it and provisions the rest.
...
# BOOT_MODULE_RESERVE_UNCHECKED=1 removes boot_module_placement_check: a module
# is recorded and verified wherever GRUB put it and nothing asks whether the
# kernel is about to write there. A control arm, never a build option:
# smoke-boot-module-image-control requires the image probe's boot to carry on,
# and smoke-boot-module-reverify-control combines it with the next flag.
...
# POOL_RESERVE_FIXED_BASE=1 puts the page pool's base reserves back at
# USER_PHYS_BASE whatever GRUB loaded there, the layout before 2026-09-19. A
# control arm, never a build option: smoke-boot-module-reserve-control requires
# the padded boot to halt on the overlap.
POOL_RESERVE_FIXED_BASE ?= 0
ifeq ($(POOL_RESERVE_FIXED_BASE),1)
CFLAGS  += -DPOOL_RESERVE_FIXED_BASE
ASFLAGS += -DPOOL_RESERVE_FIXED_BASE
endif

# BOOT_MODULE_IMAGE_PROBE=1 records a synthetic module over the first page of
# the kernel's .text before the placement check runs. GRUB never places a module
# there, so this is the only way to reach the image half of the check. A
# selftest for smoke-boot-module-reserve, never a build option. It is listed in
# DEFECT_FLAGS although it restores no defect: it fabricates a boot module, so a
# boot under it must say so rather than announce `none` (CLAUDE.md section 6).
BOOT_MODULE_IMAGE_PROBE ?= 0
ifeq ($(BOOT_MODULE_IMAGE_PROBE),1)
CFLAGS  += -DBOOT_MODULE_IMAGE_PROBE
...
# PS2_PROBE=1 paints a PS/2 liveness readout in the console's top-right corner:
# the IRQ 1 count, the last scancode, and the 8042 status register. NOT a defect
# of ours -- it is an instrument, and it is in DEFECT_FLAGS because an
# instrument that changes the machine has to say so on the wire.
ifeq ($(PS2_PROBE),1)
CFLAGS  += -DPS2_PROBE
endif
...
# STORAGE_FORMAT_SIZE_IGNORED=1 accepts the volume size the installer asked for,
# bounds it, and then lays the volume over the whole device anyway. Every call
# returns success; the defect is only visible as the size of what was written.
# The control arm for make smoke-installer-sized.
...
# free_user_physical_page fails closed (S102, HORUS-20260919-01): a double free,
...
ifdef BUILD_ID
USERSPACE_CFLAGS += -DHORUS_BUILD_ID='"$(BUILD_ID)"'
endif
...
# Does the page free path fail closed on its own? (S102, HORUS-20260919-01,
...
# BLOCKS_PER_DISK was declared here to size the test ATA image, under the rule
# "MUST match the kernel's volume". PERSIST_BLOCKS below replaced both the
# variable and the rule -- a test image is sized from the WORKLOAD, and the
# kernel's ceiling is a different number -- and this declaration was left behind
# with no readers. Removed rather than kept: a variable nothing expands is
# harmless, but a comment stating the superseded rule beside it is not, and it is
# how KEYSLOT_BLOCKS_IMG went on tracking the ceiling for another day.
else
SMOKE_FS_FLAGS =
SMOKE_FS_ENV   =
SMOKE_FS_PREP  = true
...
# ---- S96: a verified boot module is the one the kernel serves ---------------
#
...
# reserves (loader staging, the RAM vdisk, the untyped arena). Until 2026-09-19
# the reserves sat at USER_PHYS_BASE whatever GRUB put there; measured then, a
# verified bin/tcc pushed into the vdisk no longer matched its manifest hash when
# fs_server read it. #402 halted such a boot; the reserves now move clear of the
# modules (pool_reserve_base), and every verified module is hashed again once
# the reserves have been written and before userspace starts.
#
# THREE BOOTS, one per claim.
#  1. The 16 MiB pad ahead of head, seq and wc, so every module after it lands
#     above 16 MiB. The boot must reach the shell and print the re-verification
#     line, which names the reserves as MOVED: a pad that no longer pushed
#     anything past 16 MiB would print "at USER_PHYS_BASE" and fail this, rather
#     than pass without testing the move.
#  2. The same ISO through coreutils_session.py: head, seq and wc, loaded above
#     16 MiB, run from /bin. A module the kernel kept but could not serve intact
#     would fail here.
#  3. BOOT_MODULE_IMAGE_PROBE, a module recorded over the kernel's .text: the
#     boot must halt naming it and then STALL, the refusal a continuing boot
#     prints for an unmanifested module never appearing.
PAD_MODULES = COREUTILS_MODULES=1 COREUTILS_MODULE_SET="head seq wc" BOOT_MODULE_RESERVE_PAD=1

.PHONY: smoke-boot-module-reserve
smoke-boot-module-reserve:
	@$(MAKE) --no-print-directory clean
	@$(MAKE) --no-print-directory $(PAD_MODULES)
	@$(MAKE) --no-print-directory $(PAD_MODULES) horus.iso
	@SMOKE_TIMEOUT=$(SMOKE_TIMEOUT) \
		REQUIRE_MARKER='boot: all boot modules re-verified after the pool was built, with the pool reserves moved clear of them' \
		FAIL_MARKER='boot: HALT' \
		tools/smoke_test.sh horus.iso
	@SESSION_TIMEOUT=$(SMOKE_TIMEOUT) tools/coreutils_session.py horus.iso
	@$(MAKE) --no-print-directory clean
	@$(MAKE) --no-print-directory BOOT_MODULE_IMAGE_PROBE=1
	@$(MAKE) --no-print-directory BOOT_MODULE_IMAGE_PROBE=1 horus.iso
	@SMOKE_TIMEOUT=$(SMOKE_TIMEOUT) \
...
smoke-boot-module-reserve-control:
	@$(MAKE) --no-print-directory clean
	@$(MAKE) --no-print-directory $(PAD_MODULES) POOL_RESERVE_FIXED_BASE=1
	@$(MAKE) --no-print-directory $(PAD_MODULES) POOL_RESERVE_FIXED_BASE=1 horus.iso
	@SMOKE_TIMEOUT=$(SMOKE_TIMEOUT) MARKER_ONLY=1 \
		REQUIRE_MARKER="overlaps the page pool's base reserves; refusing to boot" \
		tools/smoke_test.sh horus.iso

# The falsifying arm for the RE-VERIFICATION: the reserves back on the modules AND
# the placement check gone, the pre-#402 kernel. The vdisk is formatted over the
# modules behind the pad, and the re-verification must catch it by name before
# userspace starts.
.PHONY: smoke-boot-module-reverify-control
smoke-boot-module-reverify-control:
	@$(MAKE) --no-print-directory clean
	@$(MAKE) --no-print-directory $(PAD_MODULES) POOL_RESERVE_FIXED_BASE=1 BOOT_MODULE_RESERVE_UNCHECKED=1
	@$(MAKE) --no-print-directory $(PAD_MODULES) POOL_RESERVE_FIXED_BASE=1 BOOT_MODULE_RESERVE_UNCHECKED=1 horus.iso
	@SMOKE_TIMEOUT=$(SMOKE_TIMEOUT) MARKER_ONLY=1 \
		REQUIRE_MARKER='changed after it was verified; refusing to boot' \
		tools/smoke_test.sh horus.iso

# The falsifying arm for the IMAGE half: the placement check gone, under the image
# probe. The boot carries on to the manifest, which refuses the synthetic module
# because it was never in it; the halt the base gate requires never comes.
.PHONY: smoke-boot-module-image-control
smoke-boot-module-image-control:
	@$(MAKE) --no-print-directory clean
...
gitleaks:
	...
	: "Roadmap 4.3: this exits non-zero on a finding, and is meant to."
	: "--redact because the gate now RUNS in CI on a public repository: an"
	: "unredacted finding prints the secret it just found into a world-readable"
	: "log, which turns the detector into the disclosure. gitleaks still names"
	: "the file, line and rule, which is what a fix needs."
...
cargo-audit:
	...
	: "Roadmap 4.3. This used to swallow its status into an echo naming two"
	: "conditions at once -- not installed, or no advisories found -- which is"
	: "worse than a bare true, because those two need opposite responses and a"
	: "REAL advisory printed the reassuring half and exited 0. Both now fail."
	: "Tested by asking cargo whether the subcommand RUNS, not by looking for a"
	: "binary named cargo-audit on PATH: the binary being present says nothing"
	: "about the subcommand working, and presence is not what the scan needs."
	cargo audit --version >/dev/null 2>&1 || \
	    { echo "cargo-audit is NOT INSTALLED -- that is a broken scan, not a clean one"; exit 1; }
	cd rust && cargo audit

# Header dependencies generated by -MMD (see CFLAGS). Must come after all rules.
-include $(OBJS:.o=.d) $(RUST_EXTRA_OBJS:.o=.d)

# THE TEST IMAGE'S SIZE, NOT THE KERNEL'S CEILING -- the rule PERSIST_BLOCKS
# already records, and the instance that fix missed.
#
# This tracked BLOCKS_PER_DISK, so every gate below formatted a 16 GiB volume.
# That was right while the constant WAS every volume's size and became wrong the
# moment the volume started being sized from the disk (**S68**), exactly as it did
# for the persistence gates -- but PERSIST_BLOCKS was given the fix and this was
# not, and it sits four thousand lines away with no comment saying which of the
# two rules it meant to follow.
#
# THE COST WAS NOT ACADEMIC. A format's duration scales with the volume: the
# metadata region alone is one block per 128, so 16 GiB is ~128 MiB of metadata
# plus its Merkle tree, written through emulated IDE PIO under TCG. Measured
# 2026-09-02 on `make smoke-installer`, one host, same ISO:
#
#     4194304 blocks (16 GiB)  PASS in 247s
#       32768 blocks (128 MiB) PASS in  17s
#
# 14x, against a 300s budget -- so on any runner slower than that host, boot 1
# reached `INSTALLER: formatting` and timed out. It did, twice on main
# (runs 33548784980 and 33574912969), while the SAME COMMITS passed on their
# pull-request runs, which is what a marginal budget looks like from the outside:
# a red that follows the runner rather than the diff.
#
# The budget is deliberately NOT raised to cover it. 247s of that run was a
# 16 GiB format none of these gates asks for -- not one of them reads or writes
# enough to need the space, and `smoke-fs-16g` (BIGVOL_BLOCKS) is where a
# genuinely large volume is exercised, on purpose, as the witness for S68.
KEYSLOT_BLOCKS_IMG ?= $(PERSIST_BLOCKS)
KEYSLOT_TIMEOUT    ?= 300

# Several passwords open one volume, and revoking one revokes exactly that one.
# TWO BOOTS on ONE disk image, because the property is about surviving a power
# cycle: boot 1 formats and adds a second slot, boot 2 proves both passwords
# still open it, revokes one, and proves the other is untouched.
...
# The falsifying arm. Revocation reports success and leaves the wrap openable --
# so the revoked password still unlocks the volume. Note the marker: it is the
# PASSWORD still opening, not a slot count, because the count drops either way.
...
# exactly 24576 blocks on a disk of 32768, then powers off and logs into it: a
# smaller volume is only a volume if the next boot recognises it and the
# password still opens it. The arm lays the volume over the whole disk whatever
# was asked, and must go red on the SIZE, not on anything else.
.PHONY: smoke-installer-sized smoke-installer-sized-control
smoke-installer-sized:
	@$(MAKE) --no-print-directory clean
	@$(MAKE) --no-print-directory STORAGE_ATA=1 $(SIZEDARM)
	@$(MAKE) --no-print-directory STORAGE_ATA=1 $(SIZEDARM) horus.iso
	@rm -f installer-sz.img installer-sz-serial.log
	@truncate -s $$(( $(INSTALLER_BLOCKS_IMG) * $(FS_BLOCK_SIZE) )) installer-sz.img
	@SESSION_DISK=installer-sz.img INSTALLER_MODE=sized SIZED_MIB=96 \
		SESSION_TIMEOUT=$(INSTALLER_TIMEOUT) \
		INSTALLER_FORMAT_STALL=$(INSTALLER_FORMAT_STALL) INSTALLER_FORMAT_CAP=$(INSTALLER_FORMAT_CAP) \
		SESSION_SERIAL_LOG=installer-sz-serial.log BOOT_TIMEOUT=$(INSTALLER_TIMEOUT) \
		python3 tools/installer_session.py horus.iso \
	  || { echo "[installer] ----- guest serial -----"; \
	       tail -60 installer-sz-serial.log 2>/dev/null | sed 's/^/  /'; \
	       rm -f installer-sz.img; exit 1; }
	@rm -f installer-sz.img
	@echo "[installer] PASS - a 96 MiB volume on a 128 MiB disk is exactly that, and it boots"

smoke-installer-sized-control:
	@out=$$($(MAKE) --no-print-directory smoke-installer-sized SIZEDARM=STORAGE_FORMAT_SIZE_IGNORED=1 2>&1); rc=$$?; \
	if [ $$rc -eq 0 ]; then \
	    echo "SIZED CONTROL: FAIL - the whole disk was used and the gate still passed"; \
	    echo "$$out" | tail -20 | sed 's/^/  /'; exit 1; \
	fi; \
	if ! echo "$$out" | grep -q "the volume is not the size that was chosen"; then \
	    echo "SIZED CONTROL: FAIL - it failed, but not on the size."; \
	    echo "$$out" | tail -20 | sed 's/^/  /'; exit 1; \
	fi; \
