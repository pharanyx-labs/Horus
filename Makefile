...
PROC_SELFTEST_DEP = userspace/proctest.bin userspace/exectest.bin userspace/preempttest.bin
endif
...
PIE_TEST_BINS = userspace/fsclient.bin userspace/proctest.bin userspace/exectest.bin
...
smoke-proc:
	...
	@$(MAKE) --no-print-directory boot.iso
	@SMOKE_TIMEOUT=$(SMOKE_TIMEOUT) MARKER_ONLY=1 REQUIRE_MARKER='PROC_SELFTEST: PASS exit+kill+spawn+exec' \
