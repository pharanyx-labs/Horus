...
| `IRQ_NO_MASK_ON_FIRE=1` | A registered interrupt line is left **unmasked** when it fires — what a kernel does if it treats an EOI as the end of the story. A legacy PCI line is level-triggered, so the PIC re-delivers immediately and forever: ring 3 never runs, the driver never clears the device, and the machine stops making progress permanently. **The arm for S46.** | `make smoke-net-irq-storm-control`, which asserts a **stall** rather than a marker, because that is the honest shape of a livelock: `NETTEST: PASS` reached and `NETTEST: IRQ PASS` **absent**, 3 boots in 3. Built with `IRQ_FORCE_PIC=1`: measured, QEMU storms on the 8259 and not on the I/O APIC, so this is the path where the arm can still fail |
...
| Target | Asserts |
|---|---|
| `smoke-captest` | Unheld capabilities, post-revoke use, and bad input are all refused (128 checks — read the count off `CAPTEST: PASS <n> checks` on the wire, not from here) |
