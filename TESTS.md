...
| `smoke-installer-sd-stride-control` | **Control arm, and the quiet one.** The block number handed to the card as an LBA, moving one sector where the block layer asked for eight. Nothing faults: block 0 is sector 0 either way, so the superblock reads back, the install completes and the next boot **recognises the volume**: seven of the gate's steps pass before anything is wrong. So the arm requires `the second boot recognised the volume the installer wrote` to have **passed** and the login after it to have failed, and requires `Login incorrect` on the wire rather than a timeout: the guest answers here, and a refusal read off a timeout spends the whole budget and hides a hang. A gate that stopped at the superblock would have passed on this defect. |
