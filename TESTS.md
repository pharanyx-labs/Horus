...
| `smoke-installer*` budget split (2026-09-02, **[G-13]**) | **The format has a bound of its own, and it was NOT raised.** This gate went red on `main` twice -- 2026-09-01 19:18 and 2026-09-02 00:20 -- both times a 300s timeout waiting for `INSTALLER: PASS installed`, the guest having printed `INSTALLER: formatting` and then nothing at all: no fault, no panic. It was first written off as a slow runner. **That does not survive measurement**: once the harness was made to time its own steps, the same step measured **5.7s** on a developer machine, and a CI runner is slower but not fifty times slower. So `INSTALLER_FORMAT_TIMEOUT` exists to make the format's bound separable -- raising `INSTALLER_TIMEOUT` would have loosened the refusal assertions too, where a generous budget turns a real wedge into a slow pass -- but its default stays at **300s**, because no measurement justifies more and a bigger number would hide an unexplained hang rather than fix one. The timeout message says the cause is unestablished and asks for the serial. **And the harness times its own steps now**, because sizing this from the CI logs was impossible: it buffered stdout and the runner timestamps the *flush*, so consecutive step lines land microseconds apart however long the guest took. Nobody could have argued with that 300s from a log. Filed as **[G-13]**, open. |
