...
/// Map a 32-bit pseudo-random value to a reference block index (RFC 9106 §3.4),
/// single-lane form.
fn index_alpha(pass: u32, slice: usize, index: usize, pseudo_rand: u32, seg_len: usize, lane_len: usize) -> usize {
...
/// Core Argon2id over a single lane. `mem` must hold exactly `m_blocks * 128`
/// u64 words. Writes the 32-byte tag into `out`.
fn argon2id(pwd: &[u8], salt: &[u8], t_cost: u32, m_blocks: usize, mem: &mut [u64], out: &mut [u8]) {
    // H0 (64 bytes).
    let mut h0 = [0u8; 72]; // 64-byte H0 followed by two LE32 slots
    {
        let mut h = Blake2b::new(64);
        h.update(&1u32.to_le_bytes()); // p (lanes)
    ...
    // First two blocks: B[0] = H'(H0 || LE32(0) || LE32(0)), B[1] = H'(.. LE32(1) ..).
    let mut blockbytes = [0u8; 1024];
    for (col, dst) in [0usize, 1usize].into_iter().zip([0usize, 1usize]) {
        blake2b_long(
            &mut blockbytes,
            &[&h0[..64], &(col as u32).to_le_bytes(), &0u32.to_le_bytes()],
        );
        let b = load_block(&blockbytes);
        put(mem, dst, &b);
    }

    let seg_len = m_blocks / SYNC_POINTS;
    let lane_len = m_blocks;
    let zero: Block = [0u64; QWORDS];

    for pass in 0..t_cost {
        for slice in 0..SYNC_POINTS {
            let data_independent = pass == 0 && slice < 2; // Argon2id hybrid
            // Address-block generator state for the data-independent segments.
            ...
            if data_independent && pass == 0 && slice == 0 {
                // Generate the first address block before the (index-2) loop.
                input_block[6] += 1;
            ...
            for index in start..seg_len {
                let cur = slice * seg_len + index;
                let prev = if cur == 0 { lane_len - 1 } else { cur - 1 };
                ...
                let ref_index = index_alpha(pass, slice, index, j1, seg_len, lane_len);
    ...
    // Final: tag = H'(B[last]).
    let last = get(mem, m_blocks - 1);
    let mut lastbytes = [0u8; 1024];
    for i in 0..QWORDS {
        lastbytes[i * 8..i * 8 + 8].copy_from_slice(&last[i].to_le_bytes());
    }
    blake2b_long(out, &[&lastbytes]);
}

/// Compute `m'`: the actual number of 1 KiB blocks, `= 4*floor(m_cost/4)`
/// (single lane), clamped to the Argon2 minimum of 8.
fn effective_blocks(m_cost_kib: u32) -> usize {
...
/// the pass count, parallelism is fixed at 1. `mem`/`mem_words` is a scratch
/// buffer the caller provides; it must hold at least `128 * effective_blocks(m_cost)`
/// u64 words. Writes `out_len` (== 32) bytes to `out`. Returns 0 on success,
/// -1 on bad arguments (incl. a too-small scratch buffer).
///
/// # Safety
...
pub unsafe extern "C" fn rust_argon2id_hash(
    ...
    let m_blocks = effective_blocks(m_cost);
    ...
    argon2id(pwd_s, salt_s, t_cost, m_blocks, mem_s, out_s);
    0
}
...
mod tests {
    ...
    // Reference tags from argon2-cffi (type=Argon2id, version=0x13, p=1, no
    // secret/AD) for the exact same parameters. See the generation command in
    // the commit that introduced this file.
    fn run(pwd: &[u8], salt: &[u8], m: u32, t: u32) -> [u8; 32] {
        let m_blocks = effective_blocks(m);
        let mut mem = alloc::vec![0u64; m_blocks * QWORDS];
        let mut out = [0u8; 32];
        argon2id(pwd, salt, t, m_blocks, &mut mem, &mut out);
        out
    }
    ...
    fn argon2id_vectors() {
        assert_eq!(
            hex(&run(b"password", b"0123456789abcdef", 8, 1)),
            "771338d819573c67116b39e1788ae8e04b0eb0cf9dfbbfe2e6d746cf3e464fc7"
        );
        assert_eq!(
            hex(&run(b"password", b"0123456789abcdef", 32, 3)),
            "72c63dd5c2ebcb402af0e0372451ceae01bef4a22aafdd8f22b0a234cbc5c818"
        );
        assert_eq!(
            hex(&run(b"hello world", b"saltsaltsaltsalt", 16, 2)),
    ...
    fn argon2id_kernel_profile() {
        ...
        assert_eq!(
            hex(&run(b"rootpass", b"0123456789abcdef", 4096, 3)),
