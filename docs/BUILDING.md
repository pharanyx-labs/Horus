...
| `KDIAG_SPLIT_WIDEN=1` | **Not a defect** -- the window widener: a bounded spin (`KDIAG_WIDEN_SPINS`, 60000) between the characters of a kernel marker, so a concurrently-printing task lands inside it every boot rather than at a rate. Set in **both** arms of the pair. | both arms of `make smoke-kdiag` / `smoke-kdiag-split-control` |
